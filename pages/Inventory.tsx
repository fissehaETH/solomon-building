...
const Toast = ({ message, onClose }: { message: string, onClose: () => void }) => {
  ...
  return (
    <div className="fixed bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-bottom-4 duration-300">
...
const SearchableDropdown = ({ 
  ...
  return (
    <div className="space-y-1 relative" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute z-[70] left-0 right-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
...
const Inventory: React.FC<InventoryProps> = ({ 
  ...
  const [adjustment, setAdjustment] = useState({ change: 0, reason: 'Correction' });
    ...
    <div className="space-y-6">
      ...
      <div className="sticky top-0 z-20 bg-[#F8FAFC] -mx-4 px-4 md:-mx-8 md:px-8 -mt-4 md:-mt-8 pt-4 md:pt-8 pb-4 mb-2">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            {isAdmin && (
              <button onClick={() => setShowCategoryModal(true)} className="flex-1 md:flex-none px-6 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-wider shadow-lg active:scale-95 transition-all text-xs">
                <Layers className="w-4 h-4 inline mr-2" /> Category
              </button>
            )}
            {canManageStock && (
              <button onClick={() => setShowAddModal(true)} className="flex-1 md:flex-none px-8 py-4 bg-orange-500 text-white rounded-2xl font-black uppercase tracking-wider shadow-lg active:scale-95 transition-all text-xs">
      ...
      {showAddModal && canManageStock && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
          <div className="bg-white rounded-t-[2.5rem] md:rounded-[2.5rem] w-full max-lg animate-slide-up relative h-[90vh] md:h-auto overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <button type="submit" disabled={isSubmitting || isDuplicate || !newProduct.category || !newProduct.brand} className="w-full py-5 bg-slate-900 text-white font-black uppercase rounded-2xl shadow-xl active:scale-95 transition-all">
      ...
      {showCategoryModal && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
          <div className="bg-white rounded-t-[2.5rem] md:rounded-[2.5rem] w-full max-w-lg animate-slide-up relative h-[90vh] md:h-auto overflow-y-auto">
            <div className="sticky top-0 bg-white/80 backdrop-blur-md p-8 border-b border-slate-50 flex items-center justify-between z-10">
            ...
            <form onSubmit={handleCategorySubmit} className="p-8 space-y-6">
              ...
              <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-slate-900 text-white font-black uppercase rounded-2xl shadow-xl active:scale-95 transition-all">
      ...
      {adjustingProduct && canManageStock && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 space-y-6 animate-in zoom-in-95 duration-200 shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setAdjustingProduct(null)} className="flex-1 py-5 bg-slate-100 text-slate-400 font-black uppercase rounded-2xl">Cancel</button>
